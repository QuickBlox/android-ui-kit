...
class CreateGroupDialogUseCaseTest : BaseTest() {
    @Test
    @ExperimentalCoroutinesApi
    fun createUseCase_createDialog_dialogDataIsCorrect() = runTest {
        QuickBloxUiKit.setDependency(DependencySpy())

        val dialogName = System.currentTimeMillis().toString()
        val participantIds = arrayListOf(111, 222)
        val photoUrl = System.currentTimeMillis().toString()

        val createdDialog = CreateGroupDialogUseCase(dialogName, participantIds, photoUrl).createDialog()

        assertEquals(participantIds, createdDialog.getParticipantIds())
        assertEquals(photoUrl, createdDialog.getPhoto())
        assertEquals(dialogName, createdDialog.getName())
    }

    @Test
    @ExperimentalCoroutinesApi
    fun createUseCase_createEventMessage_eventDataIsCorrect() = runTest {
        QuickBloxUiKit.setDependency(DependencySpy())

        val messageText = System.currentTimeMillis().toString()
        val dialogId = System.currentTimeMillis().toString()

        val createdEvent = CreateGroupDialogUseCase("test_name").createEvent(messageText, dialogId)

        assertEquals(messageText, createdEvent.getText())
    ...
    fun createRemoteAndSaveLocal_execute_onComplete() = runTest {
        ...
        withContext(UnconfinedTestDispatcher()) {
            runCatching {
                CreateGroupDialogUseCase("test").execute()
            }.onSuccess { result ->
    ...
    @Test
    @ExperimentalCoroutinesApi
    fun emptyDialogName_execute_onError() = runTest {
        QuickBloxUiKit.setDependency(object : DependencyStub() {
            override fun getDialogsRepository(): DialogsRepository {
                return CreateRemoteAndSaveLocalRepositoryStub()
            }
