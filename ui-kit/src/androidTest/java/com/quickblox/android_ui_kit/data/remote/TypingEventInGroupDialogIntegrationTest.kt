...
class TypingEventInGroupDialogIntegrationTest : BaseTest() {
    ...
    private fun createGroupDialog(): RemoteDialogDTO {
        val dialogDTO = RemoteDialogDTO()
        dialogDTO.type = QBDialogType.GROUP.code
        dialogDTO.participantIds = mutableListOf(OPPONENT_ID)
        dialogDTO.name = "Temp group dialog: ${System.currentTimeMillis()}"
    ...
    private fun buildQBDialog(dialogDTO: RemoteDialogDTO, chatService: QBChatService): QBChatDialog {
        ...
        qbChatDialog.name = dialogDTO.name
