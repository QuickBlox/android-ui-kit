...
class LoadUsersByNameWithExcludeByIdsUseCaseIntegrationTest : BaseTest() {
    @Before
    fun init() {
        initDependency()
        initQuickblox()
    ...
    fun paginationChangeToLoad10Users_executeWithExcludeOneUser_loaded9Users() = runBlocking {
        ...
        val users = QBUsers.getUsersByFullName("test", null).perform()
        ...
        withContext(Dispatchers.Main) {
            ...
            LoadUsersByNameWithExcludeByIdsUseCase(initPagination, "test", arrayListOf(excludeId)).execute()
                .catch { error ->
    ...
    fun paginationChangeToLoad10Users_executeTwiceWithExcludeTwoUsers_loaded18Users() = runBlocking {
        ...
        val usersA = QBUsers.getUsersByFullName("test", null).perform()
        ...
        withContext(Dispatchers.Main) {
            ...
            LoadUsersByNameWithExcludeByIdsUseCase(initPagination, "test", arrayListOf(excludeIdA)).execute()
                .catch { error ->
                    fail("expected: onCompletion, actual: catch, details: $error")
                }.onCompletion {
                    completedLatch.countDown()
        ...
        val usersB = QBUsers.getUsersByFullName("test", makeRequestBuilder(2, 10)).perform()
        ...
        withContext(Dispatchers.Main) {
            ...
            LoadUsersByNameWithExcludeByIdsUseCase(initPagination, "test", arrayListOf(excludeIdB)).execute()
                .catch { error ->
                    fail("expected: onCompletion, actual: catch, details: $error")
                }.onCompletion {
                    completedLatch.countDown()
