...
class GetDialogByIdUseCaseIntegrationTest : BaseTest() {
    ...
    fun createDialog_execute_dialogExist() = runBlocking {
        ...
        withContext(Dispatchers.Main) {
            runCatching {
                GetDialogByIdUseCase(createdDialog?.getDialogId()!!).execute()
            }.onSuccess { result ->
                loadedDialog = result
            }.onFailure { error ->
                fail("expected: Exception, actual: NotException")
