...
class AddUsersToDialogUseCaseIntegrationTest : BaseTest() {
    ...
    fun createDialogWith2Users_add1UserAndExecute_receivedDialogWith3Users() = runBlocking {
        val receivedAddUserToDialogEventLatch = CountDownLatch(1)
        subscribeToReceiveAddedUserEventInSystemMessageListener(receivedAddUserToDialogEventLatch)

        withContext(Dispatchers.Main) {
            runCatching {
                val dialogName = "test_name ${System.currentTimeMillis()}"
                val opponentIds = arrayListOf(USER_OPPONENT_ID_2)
        ...
        withContext(Dispatchers.Main) {
            runCatching {
                AddUsersToDialogUseCase(createdDialog!!, arrayListOf(OPPONENT_ID)).execute()
            }.onSuccess { result ->
        ...
        receivedAddUserToDialogEventLatch.await(10, TimeUnit.SECONDS)
        assertEquals(0, receivedAddUserToDialogEventLatch.count)

        assertEquals(3, updatedDialog?.getParticipantIds()!!.size)
        assertTrue(updatedDialog?.getParticipantIds()!!.contains(OPPONENT_ID))
    }

    private fun subscribeToReceiveAddedUserEventInSystemMessageListener(receivedMessageLatch: CountDownLatch) {
