...
class DeliverMessagesUseCaseIntegrationTest : BaseTest() {
    ...
    fun createGroupDialog_sendChatMessageAndSendDeliver_receivedDeliverMessage() = runBlocking {
        createGroupDialog()
        ...
        val deliverMessageLatch = CountDownLatch(1)
        opponentChatService?.messageStatusesManager?.addMessageStatusListener(
            StatusMessageListenerImpl(
                deliverMessageLatch
            )
        )

        DeliverMessageUseCase(loadedMessage!!).execute()

        deliverMessageLatch.await(10, TimeUnit.SECONDS)

        assertEquals(0, deliverMessageLatch.count)
    }
    ...
    @Test
    @ExperimentalCoroutinesApi
    fun createPrivateDialog_sendChatMessageAndSendDeliver_receivedDeliverMessage() = runBlocking {
        createPrivateDialog()
        ...
        val deliverMessageLatch = CountDownLatch(1)
        opponentChatService?.messageStatusesManager?.addMessageStatusListener(
            StatusMessageListenerImpl(deliverMessageLatch)
        )
    ...
    private inner class StatusMessageListenerImpl(private val deliverReadLatch: CountDownLatch) :
        ...
        override fun processMessageRead(messageId: String, dialogId: String, senderId: Int) {
            fail("expected: Delivered, actual: Read")
        }
    }
}