...
class MembersActivity : BaseActivity() {
    ...
    companion object {
        ...
        fun show(
            context: Context, dialogId: String? = null, privateChatInfoScreenSettings: MembersScreenSettings? = null
        ) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val fragment: Fragment = QuickBloxUiKit.getScreenFactory().createMembers(dialogId, screenSettings)
        supportFragmentManager.beginTransaction().replace(binding.frameLayout.id, fragment)
            .addToBackStack(MembersFragment.TAG).commit()
    }
}