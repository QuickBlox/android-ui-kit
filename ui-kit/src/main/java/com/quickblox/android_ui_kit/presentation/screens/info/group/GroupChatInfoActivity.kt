...
class GroupChatInfoActivity : BaseActivity() {
    ...
    companion object {
        ...
        fun show(
            context: Context, dialogId: String? = null, groupChatInfoScreenSettings: GroupChatInfoScreenSettings? = null
        ) {
            groupChatInfoScreenSettings?.let {
                this.screenSettings = it
            }
            val intent = Intent(context, GroupChatInfoActivity::class.java)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val fragment: Fragment = QuickBloxUiKit.getScreenFactory().createGroupChatInfo(dialogId, screenSettings)

        supportFragmentManager.beginTransaction().replace(binding.frameLayout.id, fragment)
            .addToBackStack(GroupChatInfoFragment.TAG).commit()
    }
}