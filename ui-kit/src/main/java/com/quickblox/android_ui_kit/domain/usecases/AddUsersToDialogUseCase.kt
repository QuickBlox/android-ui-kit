...
class AddUsersToDialogUseCase(private val dialogEntity: DialogEntity, private val userIds: Collection<Int>) :
    ...
    override suspend fun execute(): DialogEntity? {
        ...
        if (dialogEntity.getType() == DialogEntity.Types.PRIVATE) {
            throw DomainException("You can't add users to private dialog")
        }
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                updatedDialog = dialogsRepository.addUsersToDialog(dialogEntity, userIds)
    ...
    fun createMessageText(ownerName: String, userNames: String): String {
        ...
        val messageText = "User $ownerName added $userPrefix $userNames"
        return messageText
    }
    ...
    fun createEvent(text: String, dialogId: String): EventMessageEntity {
        ...
        eventMessageEntity.setEventType(EventMessageEntity.EventTypes.ADDED_USER_TO_DIALOG)
