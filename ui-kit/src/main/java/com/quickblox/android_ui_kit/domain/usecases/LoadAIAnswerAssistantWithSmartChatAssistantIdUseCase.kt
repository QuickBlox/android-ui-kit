...
@ExcludeFromCoverage
class LoadAIAnswerAssistantWithSmartChatAssistantIdUseCase(
    ...
    override suspend fun execute(): String {
        ...
        if (message.getContent().isNullOrEmpty()) {
            throw DomainException("The message content shouldn't be empty")
        }

        withContext(Dispatchers.IO) {
            try {
                val pagination = searchStartPagination(dialogId, message)
                val historyMessages = loadHistoryMessages(dialogId, message, pagination)

                val messageToAssistant = message.getContent()!!

                receivedAnswer = aiRepository.createAnswerWithSmartChatAssistantId(messageToAssistant, historyMessages)
